...
import json
import pandas as pd
from MicrobitesAnalytics import MicrobitesAnalytics
...
def callMicrobitesAnalytics():
    ...
    #     status_updates_json = (status_updates_df.reset_index(TIMESTAMP_COL) # index json by meal only
...
def getStatus():
    ...
    try:
        meal_id = request.args.get(meal_ID_COL)
        timestamp_mins = request.args.get(TIMESTAMP_COL)

        # Return 404 (Not Found) if the record IDs are invalid
        if (meal_id is None) or (timestamp_mins is None):
            logger.error('Missing parameters : '+meal_ID_COL+' : '+str(meal_id)
                            +', '+TIMESTAMP_COL+' : '+str(timestamp_mins))
            abort(404)

        # Read the requested meal status
        sql = ('SELECT '+meal_ID_COL+', '+TIMESTAMP_COL+', '+STATUS_LED_COL+' FROM '+ANALYTICS_TABLE+
            ' WHERE '+meal_ID_COL+' = "'+meal_id+'" AND '+TIMESTAMP_COL+' = "'+timestamp_mins+'"')
        meal_status_df = pd.read_sql_query(sql, DB_ENGINE)

        # Return 404 (Not Found) if no record is found
        if (meal_status_df is None) or (meal_status_df.empty):
            logger.error('No status found for : ' + meal_ID_COL + ' : ' + str(meal_id)
                             + ', ' + TIMESTAMP_COL + ' : ' + str(timestamp_mins))
            abort(404)
        else:
            meal_status_json = (meal_status_df
