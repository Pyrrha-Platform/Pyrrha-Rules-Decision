...
class GasExposureAnalytics(object):
    ...
    def _validate_config(self, config_filename):
        ...
        mismatched_configs_idx = [
            idx
            for idx, window in enumerate(self.WINDOWS_AND_LIMITS)
            if (window[GAS_LIMITS_PROPERTY].keys() != self.WINDOWS_AND_LIMITS[0][GAS_LIMITS_PROPERTY].keys())
        ]
        mismatched_configs = []
        if mismatched_configs_idx:
            ...
            valid_config = False
            message = (
                "%s : The '%s' for every time-window must cover the same set of gases - but these have mis-matches %s"
        ...
        if not set(self.SUPPORTED_GASES).issubset(self.WINDOWS_AND_LIMITS[0][GAS_LIMITS_PROPERTY].keys()):
            ...
            message = "%s : One or more of the '%s' %s has no limits defined in '%s' %s." % (
                config_filename,
                SUPPORTED_GASES_PROPERTY,
                str(self.SUPPORTED_GASES),
                WINDOWS_AND_LIMITS_PROPERTY,
                str(list(self.WINDOWS_AND_LIMITS[0][GAS_LIMITS_PROPERTY].keys())),
            )
            self.logger.critical(message)
            critical_config_issues += [message]
        ...
        for gas in self.SUPPORTED_GASES:
            ...
            if (min(limits) < DEVICE_RANGE_PPM[gas]["min"]) or (max(limits) > DEVICE_RANGE_PPM[gas]["max"]):
                valid_config = False
                message = (
                    "%s : One or more of the '%s' configurations %s is incompatible with the range of the '%s' device (min: %s, max: %s)."
                ) % (
                    config_filename,
                    GAS_LIMITS_PROPERTY,
                    limits,
                    gas,
                    DEVICE_RANGE_PPM[gas]["min"],
                    DEVICE_RANGE_PPM[gas]["max"],
                )
                self.logger.critical(message)
                critical_config_issues += [message]
            if (max(limits) * 2) > DEVICE_RANGE_PPM[gas]["max"]:
                ...
                message = (
                    "%s : One or more of the '%s' configurations %s is very close to the range of the '%s' device (min: %s, max: %s)."
        ...
        if not ((self.YELLOW_WARNING_PERCENT > 0) and (self.YELLOW_WARNING_PERCENT < 100)):
            ...
            message = "%s : '%s' should be greater than 0 and less than 100 (percent), but is %s" % (
                config_filename,
                YELLOW_WARNING_PERCENT_PROPERTY,
                self.YELLOW_WARNING_PERCENT,
            )
            self.logger.critical(message)
            critical_config_issues += [message]
        ...
        for gas in self.SUPPORTED_GASES:
            if (not isinstance(self.SAFE_ROUNDING_FACTORS[gas], int)) or (not (self.SAFE_ROUNDING_FACTORS[gas] >= 0)):
                ...
                message = "%s : '%s' for '%s' should be a positive integer, but is %s" % (
                    config_filename,
                    SAFE_ROUNDING_FACTORS_PROPERTY,
                    gas,
                    self.SAFE_ROUNDING_FACTORS[gas],
                )
                self.logger.critical(message)
                critical_config_issues += [message]
    ...
    def __init__(self, list_of_csv_files=None, config_filename=DEFAULT_CONFIG_FILENAME):
        ...
        if list_of_csv_files is not None:
            ...
            if not isinstance(list_of_csv_files, list):
                list_of_csv_files = [list_of_csv_files]
            dataframes = []
    ...
    def _get_block_of_device_readings(self, block_end):
        ...
        if self._from_db:
            ...
            device_log_df = pd.read_sql_query(sql, self._db_engine, parse_dates=[TIMESTAMP_COL], index_col=TIMESTAMP_COL)
        ...
        if device_log_df.empty:
            self.logger.info(
                "No 'live' device records found in range [%s to %s]" % (block_start.isoformat(), block_end.isoformat())
            )
            ...
            ff_time_spans_in_this_block_df = pd.DataFrame(
                device_log_df.reset_index().groupby(FIREFIGHTER_ID_COL)[TIMESTAMP_COL].agg(["min", "max"])
            ...
            if self._FF_TIME_SPANS_CACHE is None:
                ...
                self._FF_TIME_SPANS_CACHE = pd.merge(
                    np.fmin(ff_time_spans_in_this_block_df.loc[:, DATA_START], self._FF_TIME_SPANS_CACHE.loc[:, DATA_START]),
                    np.fmax(ff_time_spans_in_this_block_df.loc[:, DATA_END], self._FF_TIME_SPANS_CACHE.loc[:, DATA_END]),
                    how="outer",
                    on=FIREFIGHTER_ID_COL,
                )
    ...
    def _calculate_TWA_and_gauge_for_all_firefighters(self, device_log_chunk_df, ff_time_spans_df, timestamp_key):
        ...
        longest_window_df.loc[:, self.SUPPORTED_GASES] = longest_window_df.loc[:, self.SUPPORTED_GASES].mask(
            cond=(longest_window_df.loc[:, self.SUPPORTED_GASES] < 0), other=np.inf
        )
        ...
        longest_window_cleaned_df = (
            longest_window_df.sort_index()
            .groupby(FIREFIGHTER_ID_COL, group_keys=False)
            .resample(resample_timedelta)
            .nearest(limit=1)
            .sort_index()
        )
        ...
        if timestamp_key in longest_window_cleaned_df.index:
            ...
            latest_device_readings_df = (
                longest_window_cleaned_df.loc[[timestamp_key], :]  # the current minute
                .reset_index()
        ...
        for time_window in windows_in_desc_mins_order:
            ...
            overlap_delta = overlap_df.loc[:, [DATA_END, WINDOW_END]].min(axis="columns") - overlap_df.loc[
            ...
            window_twa_df = (
                window_twa_df.assign(**{TIMESTAMP_COL: timestamp_key})
                .reset_index()
            ...
            window_gauge_df = (window_twa_df.loc[:, self.SUPPORTED_GASES] * 100 / gas_limits).round(
                0
            )  # we don't need decimal precision for percentages
            ...
            window_gauge_df = window_gauge_df.add_suffix(
                (GAUGE_SUFFIX + MIN_SUFFIX) % (str(time_window[WINDOW_MINS_PROPERTY]))
            )
        ...
        everything_for_1_min_df[STATUS_LED_COL] = pd.cut(
            ...
            bins=[GREEN_RANGE_START, yellow_range_start, RED_RANGE_START, RED_RANGE_END, np.inf],
            include_lowest=True,
        ...
        gas_cols = everything_for_1_min_df.columns[
            everything_for_1_min_df.columns.str.contains("|".join(self.SUPPORTED_GASES))
        ]
        everything_for_1_min_df.loc[:, gas_cols] = (
            everything_for_1_min_df.loc[:, gas_cols].fillna(value=np.nan).replace(np.inf, RANGE_EXCEEDED)
        )
    ...
    def run_analytics(self, current_utc_timestamp=None, commit=True):
        ...
        if current_utc_timestamp.tzinfo is not None:
            current_utc_timestamp = current_utc_timestamp.tz_convert(None)
        ...
        if commit:
            analytics_df.to_sql(
                ANALYTICS_TABLE, self._db_engine, if_exists="append", dtype={FIREFIGHTER_ID_COL: FIREFIGHTER_ID_COL_TYPE}
            )
